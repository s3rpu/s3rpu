 ...
 * combates y el motor de meta funcionan juntos. No reemplaza una suite de
 * tests real, pero sirve para detectar roturas rapido (`npm test`).
 */
async function main() {
  ...
  const teamA = loadTeamFile('examples/team-flareon-core.txt');
  const teamB = loadTeamFile('examples/team-rain-sun.txt');
  ...
  const metaTeams = buildSyntheticMetaTeams(snapshot, { teamCount: 2, teamSize: 4 });
